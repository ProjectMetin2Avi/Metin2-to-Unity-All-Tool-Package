...
public class Metin2TerrainEdgeFixer : EditorWindow
{
    ...
    [Header("Advanced Texture Blending")]
    private float textureBlendStrength = 0.8f;
    private int blendSteps = 3;
    private bool useGradientBlending = true;
    private bool preserveDominantTextures = true;
    private float dominanceThreshold = 0.6f;
    private AnimationCurve blendCurve = AnimationCurve.EaseInOut(0f, 0f, 1f, 1f);

    [Header("Undo Settings")]
    private bool enableUndo = true;
    private string undoLabel = "Terrain Edge Fix";
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.HelpBox("1. Select terrains\n2. Adjust blending settings\n3. Click 'Fix Edges'", MessageType.Info);
        ...
        // Advanced Texture Blending Settings
        if (blendTextures)
        {
            EditorGUILayout.Space(5);
            EditorGUILayout.BeginVertical("box");
            GUILayout.Label("Advanced Texture Blending", EditorStyles.boldLabel);

            textureBlendStrength = EditorGUILayout.Slider("Texture Blend Strength", textureBlendStrength, 0.1f, 1f);
            EditorGUILayout.HelpBox("Higher values create smoother transitions", MessageType.Info);

            blendSteps = EditorGUILayout.IntSlider("Blend Steps", blendSteps, 1, 5);
            EditorGUILayout.HelpBox("More steps = smoother but slower blending", MessageType.Info);

            useGradientBlending = EditorGUILayout.Toggle("Use Gradient Blending", useGradientBlending);
            if (useGradientBlending)
            {
                EditorGUILayout.LabelField("Blend Curve");
                blendCurve = EditorGUILayout.CurveField(blendCurve);
            }

            preserveDominantTextures = EditorGUILayout.Toggle("Preserve Dominant Textures", preserveDominantTextures);
            if (preserveDominantTextures)
            {
                dominanceThreshold = EditorGUILayout.Slider("Dominance Threshold", dominanceThreshold, 0.3f, 0.9f);
                EditorGUILayout.HelpBox("Textures above this threshold will be preserved", MessageType.Info);
            }
