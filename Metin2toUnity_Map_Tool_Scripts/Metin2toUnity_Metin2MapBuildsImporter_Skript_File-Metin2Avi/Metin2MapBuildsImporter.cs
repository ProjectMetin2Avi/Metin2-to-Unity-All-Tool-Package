...
namespace Metin2MapTools
{
    ...
    public class Metin2MapBuildsImporter : EditorWindow
    {
        ...
        private bool createScriptableObjects = false;
        ...
        private Vector2 mapOffset = new Vector2(-64.5f, 64.5f); // X ve Z ekseni kaydırma değerleri
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                ...
                "3. Select your property folder containing .prd/.prt files\n" +
        ...
        private void ScanPropertyFiles()
        {
            ...
            // Get all property files (.prd, .prt and .prb) recursively
            string[] prdFiles = Directory.GetFiles(propertyFolderPath, "*.prd", SearchOption.AllDirectories);
            ...
            allPropertyFiles.AddRange(prdFiles);
            allPropertyFiles.AddRange(prtFiles);
            allPropertyFiles.AddRange(prbFiles);

            Debug.Log($"Found {prdFiles.Length} .prd files, {prtFiles.Length} .prt files and {prbFiles.Length} .prb files");
        ...
        private void ImportAllObjects()
        {
            ...
            float scaleFactorXZ = 131f / 256f; // 131 / 256 ≈ 0.511 scale factor for X and Z axes
            float scaleFactorY = 196.5f / 256f; // 196.5 / 256 ≈ 0.767 scale factor for Y axis
                    ...
                    if (line.StartsWith("Start Object"))
                    {
                        ...
                        if (positionData.Length >= 3)
                        {
                            try
                            {
                                ...
                                // Apply scaling
                                float scaledX = (originalX / COORDINATE_SCALE) * scaleFactorXZ;
                                float scaledZ = (-originalY / COORDINATE_SCALE) * scaleFactorXZ; // Convert Y to Z axis
                                float scaledY = (originalZ / COORDINATE_SCALE) * scaleFactorY;
                                ...
                                if (!string.IsNullOrEmpty(propertyContent))
                                {
                                    ...
                                    foreach (var propLine in propertyLines)
                                    {
                                        ...
                                        else if (trimmedLine.StartsWith("treefile") || trimmedLine.StartsWith("modelfile") || trimmedLine.StartsWith("buildingfile") || trimmedLine.StartsWith("dungeonblockfile"))
                                        {
                                ...
                                // Try to find prefab first; if not found, fall back to FBX
                                GameObject modelPrefab = null;
                                if (!string.IsNullOrEmpty(fbxModelName))
                                {
                                    // First search for a prefab with the same name
                                    modelPrefab = FindPrefabInProject(fbxModelName);
                                    if (modelPrefab == null)
                                    {
                                        // Fallback to searching raw FBX model
                                        modelPrefab = FindModelInProject(fbxModelName);
                                    }

                                    if (modelPrefab != null)
                                    {
                                        Debug.Log($"Found asset: {fbxModelName}");
                                    }
                                    else
                                    {
                                        Debug.Log($"No prefab or FBX asset found for: {fbxModelName}");
                                    }
                                }
        ...
        // Searches the project for a prefab with the given name
        private GameObject FindPrefabInProject(string prefabName)
        {
            if (string.IsNullOrEmpty(prefabName)) return null;

            // Build search paths list (reuse modelsFolderPath to optionally narrow search)
            List<string> searchPaths = new List<string>();
            if (!string.IsNullOrEmpty(modelsFolderPath))
            {
                searchPaths.Add(modelsFolderPath);
            }
            searchPaths.Add("Assets/Prefabs");
            ...
            foreach (var path in searchPaths)
            {
                ...
                {
                    string name = Path.GetFileNameWithoutExtension(prefabFile);
        ...
        // Searches the project for an FBX model with the given name
        private GameObject FindModelInProject(string modelName)
        {
            ...
            foreach (var path in searchPaths)
            {
                ...
                foreach (var fbxFile in fbxFiles)
                {
                    ...
                    if (fbxName.Equals(modelName, StringComparison.OrdinalIgnoreCase))
                    {
        ...
        // Extracts the first quoted value or the last whitespace-delimited token
