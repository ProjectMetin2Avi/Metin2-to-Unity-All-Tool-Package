...
public class Metin2TerrainTileImporter : EditorWindow
{
    ...
    private void DrawStandardModeGUI()
    {
        ...
        EditorGUILayout.HelpBox("Shift splat maps on terrain surface. Positive X moves right, positive Z moves up.", MessageType.Info);
        offsetX = EditorGUILayout.Slider("Offset X", offsetX, -10f, 10f);
        offsetZ = EditorGUILayout.Slider("Offset Z", offsetZ, -10f, 10f);
                ...
                for (int x = 0; x < size; x++)
                {
                    ...
                    for (int ky = -kr; ky <= kr; ky++)
                        for (int kx = -kr; kx <= kr; kx++)
                        {
                            ...
                            // Uygulanan offset'leri hesaplamalara dahil et
                            // offsetX ve offsetZ değerlerini texture koordinat sistemine dönüştür
                            float offsetInTextureX = (offsetX * size) / (float)rawSize;
                            float offsetInTextureZ = (offsetZ * size) / (float)rawSize;

                            float rawX = (x + kx - offsetInTextureX) * rawSize / (float)size;
                            float rawY = (y + ky - offsetInTextureZ) * rawSize / (float)size;
